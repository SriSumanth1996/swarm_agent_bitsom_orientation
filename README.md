...
<p align="center">
  <b>A Streamlit app for building stakeholder agents, running multi-round scenario debates, and generating structured final reports—for classroom and tutoring use.</b>
</p>
...
Each stakeholder agent can optionally include up to 3 supporting article URLs.

The app attempts to extract readable article text from the provided URLs and uses that extracted text as additional context for that agent during the debate.

Article extraction works best with clean, text-heavy news or article pages. Some websites may fail because of paywalls, JavaScript-heavy pages, blocked scraping, or limited readable content.
...
- Supporting article extraction is not limited to a specific publisher. It depends on whether the target website exposes readable article text.
- Some sites may block extraction or return too little usable content.
...
The MIT License applies to the source code. [BITSoM](https://www.bitsom.edu.in/) names, logos, visual identity, and institutional branding elements used in this project are not granted for reuse and should not be copied, modified, or redistributed without appropriate permission.
